...
Temos na Tabela \@ref{tab:t1} um resumo descritivo da série onde o volume útil
...
pior. Portanto, 
Na Tabela 4 s ̃ao apresentados os valores de EQM e
MAPE para os modelos no per ́ıodo de ajuste e per ́ıodo da previs ̃ao passos a frente. Verifica-se
que os valores para o modelo βARMA s ̃ao menores, tanto no per ́ıodo de ajuste quanto no
per ́ıodo da previs ̃ao. Ainda no per ́ıodo de previs ̃ao a diferen ̧ca entre os valores das medidas
consideradas  ́e maior.
...
Para garantir que os modelos satisfaçam os pressupostos teóricos, emprega-se
técnicas de visualização e testes de hipóteses, para avaliar como os resíduos 
dos modelos propostos se comportam.
...
ggplot(df_resi)+
  ...
  geom_hline(yintercept = -2,linetype="dashed",colour = "orange",linewidth=0.3)+
  geom_hline(yintercept = -3,linetype="longdash",colour = "orange",linewidth=0.3)+
  geom_hline(yintercept = 2,linetype="dashed",colour = "red",linewidth=0.3)+
  geom_hline(yintercept = 3,linetype="longdash",colour = "red",linewidth=0.3) +
  ggplot(df_res)+
...
```{r}
t3<-seq(as.Date("2013/02/01"), by = "month", length.out = 120) 
df3<-data.frame(t3,reservatorio,pred=c(fit_karma$fitted,fit_karma$forecast))
ggplot(df3)+
  geom_line(aes(x=t3,y=reservatorio))+
...
ggplot(df3)+
  ...
  geom_vline(xintercept = as.Date("2023/01/01"),linetype="dashed",colour = "black",size=0.5)+
  labs(x="Tempo (meses)",
       y="Taxa de desocupação")
