...
abstract: "O atual trabalho pretende visa utilizar a abordagem de séries temporais
com taxas e proporções para a análise do nível do volume útil na usina hidrelétrica de Passo
Fundo. Visando compreender comportamento do volume útil nos 
meses futuros, para possibilitar a definição de medidas de seguranças para garantir o fornecimento de energia. Para isso, verificou-se que o modelo com melhor ajuste foi o modelo Kumaraswamy autorregressivo de médias móveis (KARMA), modelo que utiliza a distribuição Kumaraswamy a qual possui suporte para taxas e proporções e é históricamente usados para dados hidrológicos."
header-includes:
